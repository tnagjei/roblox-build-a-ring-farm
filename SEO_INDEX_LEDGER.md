...
| Sitemap | 已确认前 20 条全是 `https://www.buildaringfarm.org/...` | www 统一通过 |
...
|---|---|---|---|---|---|
| `/codes/` | 未索引或无法知晓见解 | 待 Bing 更新 | 已请求编制索引 | 只记录，不改内容 | 3 天后复查 |
...
| `/rings/` | 已发现但未收录 | 2026-05-25 | 已请求编制索引 | 新页面，仅观察 | 3 天后复查 |
...
| `/mutations/` | CTR 候选页 | 已索引，已有 GSC 表现 | 已索引 | 已确认 | 待抽查 | 允许 | 2997 | 35 | 1.17% | 7.70 | 高曝光低点击，CTR 实验候选 | 第 14 天后只改 Title 或 Description 二选一 |
...
| `/codes/` | CTR 候选页 | 已索引，已有 GSC 表现 | 未索引，已请求编制索引 | 已确认 | 待抽查 | 允许 | 392 | 2 | 0.51% | 9.93 | 低 CTR 候选，但不能造 active codes | 3 天后复查 Bing，第 14 天再判断 CTR 实验 |
...
| `/calculator/` | 新增观察页 | 已索引 | 已索引 | 已确认出现 | 待抽查 | 允许 | 待上传 | 待上传 | 待上传 | 待上传 | 新加入台账，暂不优化 | 只观察 |
| `/rings/` | 新增观察页 | 已索引 | 未索引，已请求编制索引 | 已确认出现 | 待抽查 | 允许 | 待上传 | 待上传 | 待上传 | 待上传 | 新页面，Bing 未索引 | 3 天后复查 Bing |
...
| 优先级 | 页面 | 当前状态 | 复查时间 | 动作 |
|---|---|---|---|---|
| P0 | `/codes/` | 未索引，已请求 | 2026-05-29 | 复查 |
...
| P2 | `/rings/` | 未索引，已请求 | 2026-05-29 | 复查 |
...
| CTR 实验 `/mutations/` | 第 14 天仍高曝光低点击 | 只改 Title 或 Description 二选一 | 不同时改正文、结构、内链 |
| CTR 实验 `/codes/` | 第 14 天仍低 CTR | 优化“0 verified codes”表达 | 不造 active codes |
